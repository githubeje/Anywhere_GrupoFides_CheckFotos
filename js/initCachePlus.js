...
function InitCache() {
	...
	this.makeUpdateOnlyOneRequest = function(metodoJavascript, lastVersion) {
		...
		popup.bloqPopup("Mensaje",
				...
				{"callbackopen": function() {
					...
					init.getClaseWeb(true, "ipos_client_movil", "ClusterLocalPlus", "get", null, function(data) {
	...
	this.regeneraModeloLocal_fase2 = function(metodoJavascript) {
		...
		if(ok != true) {
			...
			var popup = new MasterPopup();
			popup.alertPopup("Error", "Ha ocurrido un error al intentar descargar el modelo actual, por favor intï¿½ntelo nuevamente <br/>" + error+ "<br/>", {"urlYes" :"menu.html" });
		}
		
		log.log("Actual SIZE:"+ (1024 * 1024 * 5 - unescape(encodeURIComponent(JSON.stringify(localStorage))).length));
		log.log("Estado Acciï¿½n:"+ok);
		log.sendToSenver();
